...
- **Modern Telemetry Stack:** Export telemetry data using EDA’s built-in Prometheus exporter and monitor alarms/deviations via the Kafka exporter.
...
> **Note:** Requires a EDA hardware license.
...
3. **Integrate Containerlab with EDA:**
   - Run:
        ...
        --topology-data ../eda-telemetry-lab/clab-eda-st/topology-data.json \
...
> **Note:** Works with any license but does not support live traffic on links.
...
- **Grafana:**  
  Visualize metrics and dashboards at http://tzutzutzu:3000. For admin tasks, use admin/admin.
...
- **Prometheus UI:**  
  Check out real-time graphs at http://fdgdfgdg:9090/graph.

---

## Traffic Generation & Control

The lab includes a robust traffic script (named **traffic.sh**) that launches bidirectional iperf3 tests between designated clients. This script assumes that persistent iperf3 servers are already running on client1 and client2.
...
For the CX variant, the topology is similar but omits the leaf/spine nodes.
